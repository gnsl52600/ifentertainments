...
public class StatisticsController {
	...
	public ResponseEntity<Map<String, Object>> statisticGoodsAjax(
		...
		// 아티스트별 굿즈샵 판매 내역을 조회한다.
		List<GoodsSalesVO> artistSalesList = statisticService.selectArtistSalesList(statisticInfoVO);
		goodsStatMap.put("artistSalesList", artistSalesList);
		
		// 기간별 판매매출 상위 5개의 아티스트를 가져오고 해당 아티스트의 5개 상위 판매 품목리스트 조회한다.
		List<GoodsSalesVO> topSalesList = statisticService.selectTopSalesList(statisticInfoVO);
		goodsStatMap.put("topSalesList", topSalesList);
		
		// 기간별, 아티스트별 가입자 성별 정보를 조회한다.
		List<MemberRateVO> gsList = statisticService.selectGenderSalesList(statisticInfoVO);
		goodsStatMap.put("gsList", gsList);
		
		// 기간별, 아티스트별 가입자 연령 정보를 조회한다
		List<GoodsSalesVO> ysList = statisticService.selectYearSalesList(statisticInfoVO);
		goodsStatMap.put("ysList", ysList);
