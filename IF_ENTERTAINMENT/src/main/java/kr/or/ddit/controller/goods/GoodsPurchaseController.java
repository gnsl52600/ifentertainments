...
//	}
	...
	public ResponseEntity<?> processPayment(@RequestBody OrderVO orderVO) {
		...
		log.info("mcList : {}", mcList);

		if (mcList.get(0).getCollectionDone() == null && totalOrderVO.getTotalAllprice() >= 200000) {
	...
	@RequestMapping(value = "/autoOrderList.do", method = RequestMethod.GET)
	public String AutoOrder(@RequestParam(name ="page", required = false, defaultValue = "1") int currentPage,
			@RequestParam(required = false, defaultValue = "goodsNo") String searchType,
			@RequestParam(required = false) String searchWord,
			Model model) {
		PaginationInfoVO<goodsBuyVO> pagingVO = new PaginationInfoVO<goodsBuyVO>();
		...
		int totalRecord = goodsPurchaseService.selectAutoOrderListCount(pagingVO);
		pagingVO.setTotalRecord(totalRecord);
		
		List<goodsBuyVO> goodsAutoBuyList = goodsPurchaseService.selectAutoOrderList(pagingVO);
		pagingVO.setDataList(goodsAutoBuyList);

		model.addAttribute("pagingVO", pagingVO);
		return "admin/goods/autoOrderList";
	}
