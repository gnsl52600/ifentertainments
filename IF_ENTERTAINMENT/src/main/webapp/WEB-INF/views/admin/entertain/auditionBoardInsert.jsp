...
<div class="gap no-gap">
	<div class="inner-bg">
		<div class="element-title">
			<h4>오디션 ${name }</h4>
			<input type="hidden" id="bfNo" name="bfNo" value="${bfNo }" />
		</div>
				...
				<div class="row">
					<div class="col-md-12">
						<label>제목</label> <input type="text" id="bfTitle" name="bfTitle"
							value="${auditionBoardVO.bfTitle }" placeholder="제목을 입력해주세요">
					</div>
					<div class="col-md-12">
						<label>내용 입력</label>
						<textarea id="bfContent" name="bfContent">${auditionBoardVO.bfContent }</textarea>
					</div>
					<div class="col-md-12">
						<div class="buttonz">
	...
	$(function() {
		...
		listBtn.on("click", function() {
			...
			swal(
					...
					'등록 취소',
					'계속 작성',
					"/entertain/audition/admin/scheduleList.do"
			);
		...
		addBtn.on("click", function() {
			...
			if ($(this).text() == "수정") {
				auditionForm.attr("action",
						"/entertain/audition/admin/update.do");
			}
			var actionUrl = "/entertain/audition/admin/insert.do";
		    var successMessage = "게시물 등록 성공!";
		    
		    if ($(this).text() == "수정") {

		        actionUrl = "/entertain/audition/admin/update.do?bfNo=" + bfNo;
		        successMessage = "게시물 수정 성공!";
		    }

		    
		    auditionForm.attr("action", actionUrl);
		...
		deleteBtn.on("click", function () {
			swal(
					...
					'삭제',
					'취소',
					"/entertain/audition/admin/delete.do?bfNo=${bfNo}"
			);
		});
	});
