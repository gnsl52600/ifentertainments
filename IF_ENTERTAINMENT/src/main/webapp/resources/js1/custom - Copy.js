...
//------ scrollbar plugin
	if ($.isFunction($.fn.perfectScrollbar)) {
		$('.side-header').perfectScrollbar();
		$('.drop-meta, .news-feed-list, .tsk-prgs-lst, .friendlist, .scl-wdgt-lst, .rcnt-activt, #lst-itms, .email-compose > form').perfectScrollbar();//-------scrollbar plugin
		$('.user-list, .todo, browser-static').parent().css({ 'max-height': "315px", 'position': "relative" }) .perfectScrollbar();
		$('.user-activity').css({ 'max-height': "304px", 'position': "relative" }) .perfectScrollbar();//-------scrollbar plugin
	}
...
if ($.isFunction($.fn.jalendar)) { 
 ...
 //Weather widget
if ($.isFunction($.fn.flatWeatherPlugin)) {
    var example = $("#today-weather").flatWeatherPlugin({
        ...
        forecast: 6, //default: 5 (0 -5) how many days you want forecast (max 3 for wunderground)
...
$(window).on('load',function(){
    "use strict";
    var img_height = $('.about-section > img').height();
    $('.purchase-now').css('height',img_height);
  });
