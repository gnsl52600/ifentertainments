...
//		log.info("### userVO : {}", userVO);
	...
	public String myPageDelete(Model model, int userNo, RedirectAttributes ra) {
		String goPage = "";
		log.info("userNo##### : {}", userNo);
		ServiceResult result = mypageService.deleteMyForm(userNo);
		if (result.equals(ServiceResult.OK)) {
			goPage = "redirect:/signin.do";
			ra.addFlashAttribute("message", "회원 탈퇴에 성공하였습니다. ");
		} else {
			goPage = "redirect:/community/mypage/list.do";
			ra.addFlashAttribute("message", "회원 탈퇴에 실패했습니다. 다시 시도해주세요 !");
		}
		return goPage;
	}
}
