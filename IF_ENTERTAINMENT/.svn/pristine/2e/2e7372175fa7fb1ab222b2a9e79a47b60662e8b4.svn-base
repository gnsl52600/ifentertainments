...
${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }${myOrderDetail.goodsMembership  }
								...
								<span class="cart-prices" style="line-height: 68px"> 
									<ins>
										<span class="woocommerce-Price-amount amount">
											<fmt:formatNumber value="${myOrderDetail.odTotalprice }" pattern="#,###"/>
										</span>
									</ins> 
								</span>
							</td>
										...
										<span class="woocommerce-Price-amount amount">
											<c:choose>
												<c:when test="${myOrderDetail.orderDelivery eq 'N'}">
													<c:choose>
														<c:when test="${myOrderDetail.goodsMembership eq 'M'}">
														...
														<c:choose>
															<c:when test="${myOrderDetail.odStatus eq '0'}">
																<span style="font-size: 13px;">
																	배송일<br>(<fmt:formatDate value="${myOrderDetail.cmplDate }" pattern="yyyy-MM-dd" />)
																</span>
				...
				<div class="total-area detail-total">
					<ul>
							...
							<c:otherwise>
								<c:forEach items="${myOrderDetailList }" var="myOrderDetail">
									<li>
										...
										<i> <fmt:formatNumber value="${myOrderDetail.odTotalprice }" pattern="#,###"/>  원</i>
									</li>
								</c:forEach>
							</c:otherwise>
						</c:choose>
						...
						<li class="order-total">
							<span style="font-size: 25px;">총 결제금액 :</span>
    ...
    <div class="modal-dialog modal-di">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="inputModalLabel">교환사유를 작성해주세요.</h5>
