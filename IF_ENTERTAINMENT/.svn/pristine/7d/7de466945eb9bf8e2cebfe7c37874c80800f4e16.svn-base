...
(function() {
	...
	// 현재 활성화된 아이템에 대한 스타일 변경
	$('.suggested-caro2').on('changed.owl.carousel', function(event) {
		setTimeout(function() {
	// 모든 아이템의 블러 효과를 초기화합니다.
		$('.owl-item').css('filter', 'blur(5px)');
	// 현재 중앙에 위치한 아이템(활성화된 아이템)의 블러 효과를 제거합니다.
		$('.owl-item.active.center').css('filter', 'none');
		}, 0);
	});

	$(document).ready(function() {
    $('.myUniqueCarousel').owlCarousel({
        items: 3,
        loop: true,
        margin: 30,
        autoplay: false,
    ...
    // Carousel 초기화 시 중앙 아이템을 제외하고 블러 처리
    setTimeout(function() {
        $('.myUniqueCarousel .owl-item').css('filter', 'blur(5px)');
        $('.myUniqueCarousel .owl-item.active.center').css('filter', 'none');
    }, 0);

    // Carousel 변경 시 중앙 아이템을 제외하고 블러 처리
    $('.myUniqueCarousel').on('changed.owl.carousel', function(event) {
        setTimeout(function() {
            $('.myUniqueCarousel .owl-item').css('filter', 'blur(5px)');
            $('.myUniqueCarousel .owl-item.active.center').css('filter', 'none');
        }, 0);
